...
from Intermediate_fusion_model import build_model
...
#    batch_size_predicted = 32
    ...
    list_of_sources = [source_drugA_tx, source_drugA_ecfp, source_drugA_pydes, source_drugB_tx, source_drugB_ecfp, source_drugB_pydes, source_cellLineSC]#, source_cellLineMT, source_cellLineCNV]
    ...
    model2 = build_model(inp_sizes, out_size)
    ...
    model_name_h5 = '../models/model4/css/model_fold_' + str(fold_i) + '.h5'
    model2.save_weights(model_name_h5)

    # save indecises
    train_inds_name = '../models/model4/css/model_train_inds_' + str(fold_i) + '.npy'
    np.save(train_inds_name, train_indices[fold_i])

    test_inds_name = '../models/model4/css/model_test_inds_' + str(fold_i) + '.npy'
    ...
    np.savetxt('../models/model4/css/model_target_' + str(fold_i) + '.csv', exp_scores, delimiter=',')

    np.savetxt('../models/model4/css/model_predicted_' + str(fold_i) + '.csv', predicted_scores, delimiter=',')
